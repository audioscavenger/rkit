...
:prechecks
IF DEFINED DEBUG echo :prechecks

for /f "tokens=*" %%x in ('where get_timelapse') DO set get_timelapse="%%~x"
IF NOT DEFINED get_timelapse  echo ERROR: get_timelapse.cmd not found & pause & exit

for /f "tokens=*" %%x in ('where sync.exe') DO set sync="%%~x"
IF NOT DEFINED sync           echo ERROR: sync.exe not found; get it here: https://docs.microsoft.com/en-us/sysinternals/downloads/sync & pause & exit

for /f "tokens=*" %%x in ('where busybox.exe') DO set busybox="%%~x"
IF NOT DEFINED busybox        echo ERROR: busybox.exe not found & pause & exit
...
:main
IF DEFINED DEBUG echo :main
del /q /f %DIRNAME%\????M.tmp 2>NUL

set /P drive=Drive to test?  [%g%%CURRENTDRIVE%%END%] 
IF DEFINED VERBOSE echo %HIGH%%k%Test size: 2GB is recommended, the larger the better. %END%
set /P nbGB=GigaBytes (GB)?  [%g%%nbGB%%END%] 
IF DEFINED VERBOSE echo %HIGH%%k%Want to stress throughput?        set block(Kb)= 100000 %END%
IF DEFINED VERBOSE echo %HIGH%%k%Want to stress latency   ?        set block(Kb)= 1 %END%
IF DEFINED VERBOSE echo %HIGH%%k%Leave default for real life test: set block(Kb)= 64 %END%
set /P block=blocksize (Kb)? [%g%%block%%END%] 
echo:
...
set /A nbBlocks=%nbGB%*1024*1024/%block%

echo GO for %nbGB%GB test on %drive%\ (%nbBlocks% * %block%k blocks)...
echo:
...
echo:
echo STARTING Write test of %nbGB%GB on %drive%
echo ------------------------------------------
echo   %END%1. Flushing disk cache:%HIGH%%k% 
sync -nobanner %drive% 2>NUL
echo   %END%2. Generating %nbGB%000M.tmp out of /dev/zero...%HIGH%%k% 
call get_timelapse start >NUL 2>&1
busybox dd if=/dev/zero of=%DIRNAME%\%nbGB%000M.tmp bs=%block%k count=%nbBlocks% >NUL 2>&1
...
echo:
echo STARTING Read test of %nbGB%GB on %drive%
echo ------------------------------------------
echo   %END%1. Flushing disk cache:%HIGH%%k% 
sync -nobanner %drive% 2>NUL
echo   %END%2. Reading %nbGB%GB from %DIRNAME%\%nbGB%000M.tmp...%HIGH%%k% 
call get_timelapse start >NUL 2>&1
busybox dd if=%DIRNAME%\%nbGB%000M.tmp of=/dev/null bs=8k >NUL 2>&1 && sync -nobanner %drive% 2>NUL
call get_timelapse %TIMESTART% %TIME% >NUL 2>&1
...
call setColors.cmd >NUL 2>NUL && goto :EOF
...
set /A writeMBs=%nbGB%00000/%totalWriteCs%
set /A readMBs=%nbGB%00000/%totalReadCs%

for /F "tokens=*" %%a in ('wmic cpu get Name ^| findstr /R .') DO set cpuName=%%a

echo %c%Disk drive %HIGH%%drive%%END%%c% speed run on %DATE% %TIME% %END%
